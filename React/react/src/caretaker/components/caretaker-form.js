...
class CaretakerForm extends React.Component {
    ...
    registerDoctor(doctor) {
        return API_USERS.postCaretaker(doctor, (result, status, error) => {
            if (result !== null && (status === 200 || status === 201)) {
                console.log("Successfully inserted caretaker with id: " + result);
                this.reloadHandler();
            } else {
                this.setState(({
                    errorStatus: status,
    ...
    // handelSelectChange = (selectedValue) => {
    //     this.setState({gender: selectedValue[0]["value"]});
    //     //this.state.formControls.dob.value = selectedValue[0]["value"];
    //     console.log(this.state.formControls.gender.value);
    // }
